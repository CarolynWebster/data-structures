    ...
    for line in file_to_open:
        ...
        if line[-1] != "I":
            if line[-1] == "Spring 2016":
                spring_16.append(" ".join(line[0:2]))
            elif line[-1] == "Winter 2016":
                winter_16.append(" ".join(line[0:2]))
            elif line[-1] == "Fall 2015":
                fall_15.append(" ".join(line[0:2]))
            elif line[-1] == "Summer 2016":
                summer_16.append(" ".join(line[0:2]))
            elif line[-1] == "G":
                ghosts.append(" ".join(line[0:2]))
...
def hogwarts_by_house(filename):
    ...
    all_students = [gryffindor, slytherin, hufflepuff, ravenclaw, dumbledores_army,
                    ghosts, instructors]
...
def all_students_tuple_list(filename):
    ...
    for line in file_to_open:
        line = line.rstrip()
        line = line.split("|")
        if line[-1] != "I" and line[-1] != "G" and line[1] != "Bones":
            student_info = (line[0] + " " + line[1], line[2], line[3], line[4])
